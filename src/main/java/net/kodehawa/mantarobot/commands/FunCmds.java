    ...
    public static class Love extends SlashCommand {
        ...
        protected void process(SlashContextMongo ctx) {
            ...
            if (ctx.getOptionAsUser("otheruser") != null) {
                ...
                toDisplay = ("\uD83D\uDC97  %s%n\uD83D\uDC97  %s".formatted(Utils.getTagOrDisplay(user), Utils.getTagOrDisplay(otherUser)));
            } else {
                ids[0] = ctx.getAuthor().getIdLong();
                ids[1] = user.getIdLong();
                toDisplay = ("\uD83D\uDC97  %s%n\uD83D\uDC97  %s".formatted(Utils.getTagOrDisplay(user), Utils.getTagOrDisplay(ctx.getAuthor())));
            }
