...
public class OpenRamAPIRequester extends ActionAPIRequester{
    ...
    public ActionAPIObject getRandomImageByType(String type, boolean nsfw, String filetype) throws JsonProcessingException {
        Pair<String, String> rv = getRandomImageByType(type, nsfw);
        String extension = rv.getKey().substring(rv.getKey().lastIndexOf("."));

        return new ActionAPIObject(rv.getValue(), rv.getKey(),  extension, false, type, List.of());
    }
}
